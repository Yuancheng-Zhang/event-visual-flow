...
%% input: t_x_y_p: time, x, y, polarity (1 or -1)
%% input: mode: 0-> show only negative events; 1-> only positive events; 2-> both;

t_x_y_p = events;
mode = 2;
ev_plot_name = strcat('./datafolder','/Events3dPlot');
[s1,s2] = size(t_x_y_p);
%if s1 ~= 4
%    t_x_y_p = t_x_y_p';
%end
...
figure;clf;
if mode == 0 || mode == 2
    scatter3(t_x_y_p.y(neg_rid),t_x_y_p.x(neg_rid),t_x_y_p.ts(neg_rid),'.','MarkerEdgeColor','b','MarkerEdgeAlpha',.7);
end
if mode > 0
    hold on
